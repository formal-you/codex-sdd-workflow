...
长会话不要只靠聊天记忆推进。当前目标、已完成内容、剩余工作、下一步、风险、改动文件和验证状态，必须落到 active task、`SDD/docs/progress.md` 或 `SDD/state/hot/`。如果 UI 显示 context 使用比例达到或超过 29%，主 agent 应先写 checkpoint，再询问用户：开启压缩，还是切换到新的 session。
...
- 默认 `manual` 模式下不要自动提交
- 记录 `commit status: 未提交` 与推荐 commit message
